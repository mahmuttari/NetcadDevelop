...
ok('0 example_2000.dwg yüklendi (283 ilkel)', await page.evaluate(() => window.dwgApp.state.hasDoc && window.dwgApp.state.prims.length === 283), String(await page.evaluate(() => window.dwgApp.state.prims.length)));   // katı ve ağ gövdeleri v7.24'ten beri üçgen başına ilkel değil, gövde başına tek ağ ilkeli (k=5)
...
if (step === 'all' || step === 'compare') {
  ...
  ok('3 karşılaştırma 2000 ↔ 2018: 39 çıkan, 1 eklenen, 234 ortak', st.removed === 39 && st.added === 1 && st.common === 234, JSON.stringify(st));   // MULTILEADER çizgisi ve ok başı iki sürümde de eşleşir; katı/ağ gövdeleri tek ağ ilkeli olduğu için ortak sayısı 290 değil 234
