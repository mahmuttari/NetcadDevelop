...
// ---------------------------------------------------------------------------------
{
  ...
  ok('8a nokta isteminde istem satırında yalnız TT ve Ortho (30 px — yoğun kip, v7.82); istem tek satırda, çubuk < 100 px; "Yakalama izi" karosu Ölçü ve Ekran sekmelerinde, basılı (açık)', !b0.ttHidden && b0.ttW === 30 && b0.oW === 30 && b0.barH < 100 && b0.textH < 30 && !!b0.tile && b0.tile.on && b0.tile.lb === 'Yakalama izi' && b0.tile.icon && b0.tileD, J(b0));
  await ev(() => window.dwgApp.__trackAdd(50300, 50600, 'end'));
...
// ---------------------------------------------------------------------------------
{
  ...
  ok('10a OTRACK notu iz noktasını, TT düğmesini ve kesişimi söyler; komut sayıları sabit', /tracking point/.test(a.note || '') && /TT/.test(a.note || '') && /intersections/.test(a.note || '') && a.st.total === 508 && a.st.names === 755, J(a));
