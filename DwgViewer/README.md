# DWG Görüntüleyici (Android) — v7.4
...
| Alan | Özellik |
|---|---|
| **Yeni uygulama simgesi (v7.4)** | Uyarlanabilir simge baştan tasarlandı: lacivert dikey gradyan ve ince plan ızgarası üzerinde katlı köşeli beyaz çizim yaprağı; yaprakta L duvarlı plan, iç duvar ve kapı yayı, amber ölçü çizgisi ve amber **"DWG" alt bandı** (harfler yazı tipi gerektirmeyen çizgi tabanlı geometrik yol). İçerik 66 dp güvenli dairenin içinde kalır; yuvarlak, kare-yuvarlak ve yuvarlatılmış kare maskelerde kırpılmaz, 36 px'te de okunur. Katmanlar `drawable/ic_launcher_background.xml` (gradyan `aapt:attr`), `ic_launcher_foreground.xml`, `ic_launcher_monochrome.xml` (Android 13+ temalı simge, kontur). Ana ekrandaki marka rozeti de aynı işareti kullanır (`<symbol id="i-appmark">`). Play Store için `release/icon_512.png` (tam kare, saydamlık yok) ve maske önizlemesi `release/icon_preview.png`; üretici betik tek kaynaktan hem SVG hem VectorDrawable yazar |
