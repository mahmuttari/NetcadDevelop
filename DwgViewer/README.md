# DWG Görüntüleyici (Android) — v7.7
...
| `test_doc.mjs` | Word 97-2003 (.doc): `samples/doc/` derlemi (Apache POI sınama verisi) Node'da doğrudan `docToHtml` ile — metin iddiaları çözümleyiciden bağımsız (parça tablosu / `strings` çıktısından seçilmiş cümleler), listeler, iç içe tablo, birleştirilmiş hücreler, alan kodu gizleme, köprü, Kiril metin, dipnot / sonnot, resim, Word 6 / 95 uyarısı, sayfa boyutu, süre eşiği, bozuk / kısaltılmış dosyalarda çökmeme; tarayıcıda `#fileInput` ile açma: belge kipi, "Word 97-2003" etiketi, sayfa görünümü, akış görünümü, yakınlaştırma, resim yüklenmesi, uyarı satırı, Dosya Aç tür süzgeci. Ekran görüntüleri: `doc_sample.png`, `doc_lists.png`, `doc_picture.png` |
