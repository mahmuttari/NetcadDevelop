# DWG Görüntüleyici (Android) — v7.5
...
| Alan | Özellik |
|---|---|
| **Yeni uygulama simgesi (v7.4 – v7.5)** | Uyarlanabilir simge "yelpaze belge yığını" olarak tasarlandı: koyu mavi gradyan üzerinde önde **mavi çizim paftası** (ince ızgara, L duvarlı plan, iç duvar, kapı yayı, amber ölçü çizgisi, amber **"DWG" etiketi**), arkada merkez etrafında 7° ve 14° döndürülüp sağ üste kaydırılmış iki beyaz yaprak — üst bantları **kırmızı (PDF)** ve **turuncu (arşiv)**: uygulamanın DWG dışında PDF, Word, Excel ve arşiv de açtığını anlatır. Bütün köşeler 66 dp güvenli dairenin içindedir (en uzak köşe 32,7 dp); yuvarlak, kare-yuvarlak ve yuvarlatılmış kare maskelerde kırpılmaz, 36 px'te okunur. Harfler yazı tipi gerektirmeyen çizgi tabanlı geometrik yollardır (D, W, G). Katmanlar `drawable/ic_launcher_background.xml` (gradyan, `aapt:attr`), `ic_launcher_foreground.xml` (döndürülmüş gruplar), `ic_launcher_monochrome.xml` (Android 13+ temalı simge; arka yaprakların konturu ön paftanın dışında kalacak şekilde `clip-path` deliğiyle çizilir). Ana ekrandaki marka rozeti aynı işareti kullanır (`<symbol id="i-appmark">`). Play Store için `release/icon_512.png` (tam kare, saydamlık yok), maske ve boyut önizlemesi `release/icon_preview.png`; üretici betik tek kaynaktan hem SVG hem VectorDrawable yazar |
