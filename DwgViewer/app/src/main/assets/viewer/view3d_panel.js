...
export function renderZScale(el, v3, host) {
  ...
  el.innerHTML = W.sec('zscale', tt('v3ZScale', 'Düşey abartı'),
    `<div class="slider" data-key="zScale"><button type="button" class="dec" aria-label="${esc(tt('decrease', 'Azalt'))}"><svg class="ic" aria-hidden="true"><use href="#i-minus"/></svg></button><input type="range" data-key="zScale" data-log="10" min="-1" max="1.301" step="0.01" value="${Math.log10(z)}" aria-label="${esc(tt('v3ZScale', 'Düşey abartı'))}"><output>×${esc(fmtDef(z, 2))}</output><button type="button" class="inc" aria-label="${esc(tt('increase', 'Artır'))}"><svg class="ic" aria-hidden="true"><use href="#i-plus"/></svg></button></div>` +
